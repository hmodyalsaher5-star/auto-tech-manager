...
import { ClipboardList, PackageCheck, User, MapPin, X, Banknote, FileText, Trash2, CarFront, ListTree } from 'lucide-react';
...
export default function OrderPreparation() {
  ...
  // 💡 التعديل هنا: دمجنا جلب البيانات داخل الـ useEffect لكي لا يعترض المحرر
  useEffect(() => {
  ...
  // دالة مساعدة لتحديث القائمة بعد التعديل أو الإلغاء
    ...
    <div className="p-4 md:p-8 animate-fadeIn text-right" dir="rtl">
      ...
      {orders.length === 0 ? (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
          {orders.map((order) => (
            <div key={order.id} onClick={() => setSelectedOrder(order)} className="cursor-pointer bg-black/40 border border-teal-500/30 p-6 rounded-[2rem] hover:border-teal-400 transition-all flex flex-col justify-between">
              <div>
                  <div className="mb-4 border-b border-white/10 pb-4">
                     ...
                     <p className="text-sm text-gray-300"><MapPin className="w-4 h-4 inline text-rose-400" /> {order.governorate} - {order.region}</p>
                  </div>
                  <div className="space-y-3 mb-4 text-sm text-orange-50">
                      ...
                      <div className="mt-3 pt-3 border-t border-white/5 flex justify-between">
                          <span className="text-gray-400"><Banknote className="w-4 h-4 inline text-emerald-500" /> السعر:</span>
      ...
      {selectedOrder && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md">
          <div className="bg-black border-2 border-teal-500/50 p-6 rounded-[2rem] max-w-xl w-full max-h-[90vh] overflow-y-auto relative">
            ...
            <div className="bg-white/5 p-4 rounded-2xl mb-4">
              ...
              <p className="text-teal-300 text-sm" dir="ltr">{selectedOrder.phone1} {selectedOrder.phone2 && `| ${selectedOrder.phone2}`}</p>
            </div>
            ...
            <div className="bg-amber-900/30 p-4 rounded-2xl border border-amber-500/50 mb-4">
                <label className="text-amber-400 font-bold mb-2 flex items-center gap-2 text-sm"><FileText className="w-4 h-4" /> رقم وصل التوصيل (إجباري)</label>
                <input type="text" value={trackingNumber} onChange={(e) => setTrackingNumber(e.target.value)} placeholder="رقم الوصل..." className="w-full p-3 rounded-xl bg-black/60 border border-amber-500/50 text-white font-bold outline-none focus:border-amber-400" />
            </div>

            <div className="flex flex-col gap-2">
              <button onClick={() => handleConfirmPreparation(selectedOrder.id)} className="w-full bg-emerald-600 hover:bg-emerald-500 text-black py-3 rounded-2xl font-black text-lg shadow-lg">تأكيد التغليف والتحويل للمندوب</button>
              <button onClick={() => handleCancelOrder(selectedOrder.id)} className="w-full bg-rose-900/40 hover:bg-rose-600 text-rose-300 py-3 rounded-2xl font-bold border border-rose-500/30">إلغاء الطلب نهائياً</button>
            </div>
          </div>
        </div>
      )}
